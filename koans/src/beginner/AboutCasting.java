...
public void classCasting() {
    try {
        // Create an instance of a class that implements Sleepable
        Sleepable sleeper = new SleepableImpl();
        // Cast it to Sleepable and call the sleep method
        sleeper.sleep();
    } catch (ClassCastException x) {
        fail("Object does not implement Sleepable, maybe one of the people classes do?");
    }
...
// Define a class that implements Sleepable
class SleepableImpl implements Sleepable {
    public String sleep() {
        return "zzzz";
    }
}
